...
def posemb_sincos_3d(patches, temperature: int = 10000, dtype="float32"):
    f, h, w, dim = ops.shape(patches)[1], ops.shape(patches)[2], ops.shape(patches)[3], ops.shape(patches)[4]
    z, y, x = ops.meshgrid(ops.arange(f),
                        ops.arange(h), 
                        ops.arange(w), indexing="xy")
    fourier_dim = dim // 6
    ...
    pe = ops.concatenate((ops.sin(x), ops.cos(x), ops.sin(y), ops.cos(y), ops.sin(z), ops.cos(z)), 1)
    to_pad = dim - (fourier_dim * 6)
    pe = ops.pad(pe, ((0, 0), (0, 0), (0, 0), (0, to_pad))) # pad if feature dimension not cleanly divisible by 6
...
def SimpleViT3D(
    image_size, image_patch_size, frames, frame_patch_size, num_classes, dim, depth, heads, mlp_dim, channels = 3, dim_head = 64
):
    ...
    nf, nh, nw = frames // frame_patch_size, image_height // patch_height, image_width // patch_width
    patch_dim = channels * patch_height * patch_width * frame_patch_size

    i_p = layers.Input((frames, image_height, image_width, channels))
    tubelets = layers.Reshape((frame_patch_size, nf, patch_height,nh , patch_width, nw, channels))(i_p)
    ...
    tubelets = layers.LayerNormalization()(tubelets)
    pos_embedding = posemb_sincos_3d(
        tubelets
    )
